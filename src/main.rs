...
use truck_modeling::*;
use truck_polymesh::stl::{STLType, self};
...
fn main() {
    ...
    // This takes a really long time, sadly
    println!("Rasterizing...");
    // Appears to use .map() to call triangulation against every shell in the shape.
    // Would parallel map speed this up? See: https://docs.rs/rayon/latest/rayon/
    let rasterized = bottle.triangulation(0.0001).unwrap();
    println!("Converting to polygon. mesh..");
    let mesh = rasterized.to_polygon();
    println!("Done!");
    let output = "bottle.stl";
    let mut file = std::fs::File::create(&output).unwrap();
    stl::write(&mesh, &mut file, STLType::ASCII).unwrap();
}